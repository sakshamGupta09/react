...
- We can pass a config object to axios.
...
- Instead of using the `axios` prebuilt instance, we can create a custom instance.
- To preconfigure some properties of request config like `baseUrl`, `timeout` which otherwise we will have to pass to every config call.
