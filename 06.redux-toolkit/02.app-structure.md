...
- Redux toolkit has a method called `createAsyncThunk` that manages dispatching of actions automatically to know the state of promise.
- Before a call is made an `pending` action is dispatched and after the response/error, `fulfilled` or `rejected` is dispatched.
